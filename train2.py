...
def evaluation(net,val_dataloders,loss,criterion,args):
    ...
    start=time.time()
    with torch.no_grad():
        for batch, (pr,hr,zg,t,_,_) in enumerate(val_dataloders):
            pr,hr,zg ,t= prepare([pr, hr, zg,t],device,args)
            sr = net(pr,zg,t)
...
def main():
    ...
    parser.add_argument('--nEpochs', type=int, default=41, help='number of epochs to train for')
...
#    parser.add_argument('--test_end_time', type=bool, default=datetime(2012,12,24),help='')
    ...
    parser.add_argument('--start_date', type=bool, default=datetime(2005,1,2),help='a?')
    parser.add_argument('--end_date', type=bool, default=datetime(2011,12,24),help='')
...
#         transforms.Resize((316, 376)),
    ...
    data_set=ACCESS_BARRA_vdsr_zg(args.start_date,args.end_date,lr_transform=lr_transforms,hr_transform=hr_transforms,args=args)
    if args.test:
        print(1)
    else:
    ...
    #     optimizer_my = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9)
        ...
        for e in range(args.nEpochs):
            loss=0
            start=time.time()
            for batch, (pr,hr,zg,t,_,_) in enumerate(train_dataloders):
                pr,hr,zg,t= prepare([pr,hr,zg,t],device,args)
                optimizer.zero_grad()
                ...
                if batch%10==0:
                    state = {'model': net, 'sr':sr[0],'optimizer': optimizer.state_dict(), 'epoch': e}
